...
function fixtureFetch({ next = null, customDeploymentId = "dpl_old", link = null, accountId = "team_test" } = {}) {
  const urls = [];
  const responses = new Map([
    ["https://api.vercel.com/v9/projects/prj_test?teamId=team_test", { id: "prj_test", name: "release-forms", accountId, link }],
...
test("rollback capture records the complete exact pre-promotion alias map", async () => {
  ...
  assert.equal(inventory.org_id, "team_test");
...
test("rollback capture rejects pagination, mixed deployments, and Git linkage", async () => {
  ...
  await assert.rejects(() => captureRollbackInventory({ ...options, fetchImpl: fixtureFetch({ accountId: "team_other" }).fetch }), /dedicated non-Git project/);
  await assert.rejects(() => captureRollbackInventory({ ...options, expectedOrgId: "team_other", fetchImpl: fixtureFetch().fetch }), /configuration is incomplete/);
});
